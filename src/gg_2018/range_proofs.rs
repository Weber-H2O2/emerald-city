...
impl AliceProof {
    ...
    pub fn verify(
        ...
        if self.s1 > ECScalar::group_order().pow(3) {
            return false;
        }
    ...
    pub fn generate(
        ...
        let e = if check {
            let (X, u) = {
                ...
                let alpha = ECScalar::from(&round1.alpha);
...
impl BobProofExt {
    pub fn verify(
        ...
        let (x1, x2) = {
            let ec_gen = Point::generator();
            let s1 = ECScalar::from(&self.proof.s1);
            let e = ECScalar::from(&self.proof.e);
...
pub(crate) mod tests {
    ...
    fn alice_zkp() {
        ...
        let a = ECScalar::random().to_bigint();
