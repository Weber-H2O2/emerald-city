...
use emerald_city::curv::cryptographic_primitives::proofs::sigma_correct_homomorphic_elgamal_enc::HomoELGamalProof;
use emerald_city::curv::cryptographic_primitives::proofs::sigma_dlog::DLogProof;
use emerald_city::curv::cryptographic_primitives::secret_sharing::feldman_vss::VerifiableSS;
...
use emerald_city::gg_2018::party_i::*;
use emerald_city::paillier::*;

use emerald_city::curv::arithmetic::num_bigint::from;
...
fn main() {
    ...
    let message_bn = from(message);
    // let message_bn = message_bn.modulus(&two.pow(256));
    let one = BigInt::one();
    let two_at_256 = BigInt::from(2 as u64).pow(256 as u16);
    let message_bn = message_bn.modpow(&one, &two_at_256);    
    let local_sig =
    ...
    let sign_json = serde_json::to_string(&(
        "r",
        sig.r.to_big_int().to_str_radix(16),
        "s",
        sig.s.to_big_int().to_str_radix(16),
    ))
    .unwrap();
