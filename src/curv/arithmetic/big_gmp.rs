...
impl Converter for Mpz {
    ...
    fn to_hex(&self) -> String {
        self.to_str_radix(HEX_RADIX)
    }

    fn from_hex(value: &str) -> Mpz {
        BigInt::from_str_radix(value, HEX_RADIX).expect("Error in serialization")
    }
}
...
impl BitManipulation for Mpz {
    fn set_bit(self: &Self, bit: usize, bit_val: bool) -> Self {
        let mut clone_self = self.clone();
        if bit_val {
            clone_self.setbit(bit);
        } else {
            clone_self.clrbit(bit);
        }
        clone_self
    }
